...
ax1[2].plot(inclinationAngle,c='k')
...
#begin modern poly fitting
fig7 = plt.figure()


x1 = slipAngle[Z1]
y1 = lateralForce[Z1]
curve1 = np.polyfit(x1,y1,10)
...
#begin magic formula turds
alpha = slipAngle  # slip angle (deg)
C = 1  # cornering stiffness (kg/deg) --> is the slope of the SA vs Fy curve at a given point
mu_y = 0.2  # lateral friction coefficient (0-dim) --> need a better number for this
Z = verticalLoad  # verticalLoad (N)

#alpha_n = (C * math.atan(alpha))/(mu_y/Z)  # normalized slip angle
...
#phi = ((1-Ep)*alpha_n) + ((Ep/Bp)*math.atan(Bp * alpha_n))
#theta = Cp * (math.atan(Bp * phi))

#F_magic = Dp * math.sin(theta)  #magic formula
